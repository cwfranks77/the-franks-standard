...
Deno.serve(async (req) => {
  ...
  const scan = await scanAndEnforceViolation({
    ...
    metadata: { reason, listing_id: listingId },
  })
  if (scan.violated && (scan.action === 'account_frozen' || scan.action === 'fraud_case_opened')) {
    return json({ error: 'report_blocked', message: 'Report could not be submitted.' }, 403)
  }
