...
function orderIdFromMetadata (metadata?: Record<string, string> | null): string {
  return metadata?.order_id ?? metadata?.orderId ?? ''
}
...
async function handlePaymentIntentSucceeded (pi: {
  ...
  const orderId = orderIdFromMetadata(pi.metadata)
  if (!orderId) return
...
async function handlePaymentIntentFailed (pi: {
  ...
  const result = await markOrderCancelled(admin, orderId)
  if (!result.ok) {
    throw new Error(result.error ?? 'markOrderCancelled failed')
  }
}
...
async function handleCheckoutCompleted (session: {
  ...
  const shipName = session.shipping_details?.name ?? session.customer_details?.name ?? null
  const result = await markOrderPaid(admin, {
    orderId,
...
async function dispatchEvent (event: Stripe.Event) {
  switch (event.type) {
    ...
    case 'payment_intent.succeeded':
      await handlePaymentIntentSucceeded(event.data.object as Parameters<typeof handlePaymentIntentSucceeded>[0])
      break
    case 'payment_intent.payment_failed':
      await handlePaymentIntentFailed(event.data.object as Parameters<typeof handlePaymentIntentFailed>[0])
      break
    default:
      break
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!claimed) {
      return json({ received: true, skipped: true, type: event.type })
    }
