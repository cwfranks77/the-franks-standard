        ...
        <form class="sell-form" @submit.prevent="submitListing">
          ...
          <div v-if="requiresCoa" id="coa-section" class="form-section coa-section">
            ...
            <template v-if="form.coaType === 'franks_issued'">
              ...
              <p class="text-muted small mt-2">{{ COA_NON_TRANSFERABLE_NOTICE }}</p>
            ...
            <div v-if="form.coaType === 'upload'" class="mt-3 coa-upload-block">
              ...
              <label v-else class="label mt-2">Serial number on your COA (required)</label>
              <input
                v-model="thirdPartyCoaSerial"
                type="text"
                class="input"
                placeholder="e.g. PSA #12345678 or issuer serial"
                autocomplete="off"
              />
              <p class="text-muted small">This number is stored in our database with your seller account and cannot be moved to another listing.</p>
              <label v-if="photoFiles.length >= 1" class="coa-compare-check mt-2">
...
import { FRANKS_COA_OPTION_LEAD, COA_NON_TRANSFERABLE_NOTICE } from '~/utils/franksCoaModel.js'
import { sha256HexFromFile } from '~/utils/coaFileHash.js'
...
async function submitListing() {
  ...
  if (needsCoa) {
    ...
    if (form.coaType === 'upload') {
      ...
      if (String(form.description || '').trim().length < 20) {
        alert('Add a brief item description (at least 20 characters) before registering your COA.')
        return
      }
      if (!coaFile.value) {
      ...
      if (!String(thirdPartyCoaSerial.value || '').trim() || thirdPartyCoaSerial.value.trim().length < 3) {
        alert('Enter the serial number printed on your COA document.')
        return
      }
    ...
    if (form.coaType === 'franks_issued') {
      if (String(form.description || '').trim().length < 20) {
        alert('Add a brief item description (at least 20 characters) before issuing a Franks COA.')
        return
      }
    }
  ...
  try {
    ...
    if (imgUpdErr) {
      throw new Error(imgUpdErr.message)
    }
    ...
    if (effectiveCoaType === 'upload' && coaFile.value) {
      ...
      const fileHash = await sha256HexFromFile(cf)
      const { data: { session } } = await supabase.auth.getSession()
      const fnBase = String(useRuntimeConfig().public.supabaseUrl || '').replace(/\/+$/, '')
      const regRes = await fetch(`${fnBase}/functions/v1/register-coa-upload`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          storage_path: coaPath,
          mime_type: cf.type || '',
          file_size_bytes: cf.size || 0,
          file_hash_sha256: fileHash,
          third_party_serial: thirdPartyCoaSerial.value.trim(),
        }),
      })
      const regData = await regRes.json().catch(() => ({}))
      if (!regRes.ok) {
        throw new Error(regData.message || regData.error || 'COA registration failed — document not authenticated.')
      }
    }
