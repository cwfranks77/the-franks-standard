...
function applyFiltersFromRoute () {
  const qSearch = String(route.query.q || '').trim()
  if (qSearch) searchQuery.value = qSearch
...
watch(() => [route.query.q, route.query.category, route.query.limited, route.query.collection], () => {
  applyFiltersFromRoute()
})
