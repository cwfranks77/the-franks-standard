...
function pickTool (id) {
  ...
  nextTick(() => {
    const panel = document.getElementById('owner-tool-panel')
...
<template>
  <div class="grid lg:grid-cols-[240px_1fr] gap-6 items-start">
    <aside class="space-y-4 relative z-10 lg:sticky lg:top-4 max-h-[70vh] lg:max-h-[calc(100vh-6rem)] overflow-y-auto" aria-label="Operator tools menu">
      <div
    ...
    <div
      id="owner-tool-panel"
      tabindex="-1"
      class="bg-surface2 border border-border rounded-lg p-5 min-h-[320px] relative z-0 scroll-mt-24"
    >
      <slot />
    </div>
  </div>
