...
/** Operator dispute AI triage — calls Supabase ops-dispute-triage edge function. */
export function useDisputeTriageOps () {
  const config = useRuntimeConfig()
  const busy = ref(false)
  const lastError = ref('')

  async function callTriageOps (action: string, payload: Record<string, unknown> = {}) {
    busy.value = true
    lastError.value = ''
    ...
    try {
      const res = await fetch(`${base}/ops-dispute-triage`, {
        method: 'POST',
  ...
  return { busy, lastError, callTriageOps }
}
