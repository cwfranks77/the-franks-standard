/**
 * Message sanitization — strip PII, links, off-platform contact; log violations.
 */
...
function sanitizeMessage (text) {
  ...
  return {
    ...
    original_length: String(text || '').length,
  }
}

async function sanitizeAndLog (admin, {
  userId,
  content,
  sourceType = 'message',
  sourceId = null,
  ...
  if (result.modified && admin && userId) {
    await admin.from('violation_events').insert({
      ...
      severity: 'minor',
      action_taken: 'logged',
      content_excerpt: String(content).slice(0, 500),
...
module.exports = { sanitizeMessage, sanitizeAndLog, STRIP_RULES }
