...
const pageDirs = [
  ...
  path.join(root, 'bc-performance-audio', 'src', 'pages'),
...
function routeFromPage (rel) {
  let r = rel
    .replace(/\\/g, '/')
    .replace(/^bc-performance-audio\/src\/pages\//, '')
    .replace(/^pages\//, '')
    .replace(/\.vue$/, '')
...
for (const pagesDir of pageDirs) {
...
function ok (p) {
  ...
  if (crossSiteRoutes.has(norm)) return true
