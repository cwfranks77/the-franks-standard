import { buildSellerGoogleSearchUrl } from '~/utils/sellerGoogleSearch.js'

const GENERIC = /^(seller|unknown|not available|ebay)$/i

function cleanUsername (raw) {
  let u = String(raw || '').trim().replace(/^@/, '')
  try {
    u = decodeURIComponent(u)
  } catch {
    /* keep as-is */
  }
  u = u.replace(/^https?:\/\/(www\.)?ebay\.com\/usr\//i, '')
  u = u.split(/[/?#&]/)[0]
  u = u.replace(/[^\w.-]/g, '')
  if (!u || u.length < 2 || u.length > 64) return null
  if (GENERIC.test(u)) return null
  if (!/^[A-Za-z0-9._-]+$/.test(u)) return null
  return u
}

function addRow (seen, rows, username) {
  const u = cleanUsername(username)
  if (!u) return
  const key = u.toLowerCase()
  if (seen.has(key)) return
  seen.add(key)
  rows.push({
    username: u,
    profile_url: `https://www.ebay.com/usr/${encodeURIComponent(u)}`,
    store_url: `https://www.ebay.com/sch/i.html?_ssn=${encodeURIComponent(u)}&_ipg=60&rt=nc`,
    google_url: buildSellerGoogleSearchUrl(u),
  })
}

/** Pull usernames from eBay URLs embedded in arbitrary text. */
function scanEbayUrls (text, seen, rows) {
  let m
  const usrRe = /ebay\.com\/usr\/([A-Za-z0-9._%-]{2,64})/gi
  while ((m = usrRe.exec(text)) !== null) addRow(seen, rows, m[1])
  const ssnRe = /[?&]_ssn=([A-Za-z0-9._%-]{2,64})/gi
  while ((m = ssnRe.exec(text)) !== null) addRow(seen, rows, m[1])
}

/** First token on a line that looks like an eBay username. */
function usernameFromLine (line) {
  const trimmed = String(line || '').trim()
  if (!trimmed) return null

  const usr = trimmed.match(/ebay\.com\/usr\/([^/?#&\s]+)/i)
  if (usr) return usr[1]

  const ssn = trimmed.match(/[?&]_ssn=([A-Za-z0-9._%-]{2,64})/i)
  if (ssn) return ssn[1]

  const parts = trimmed.split(/[,;\t|]+/)
  for (const part of parts) {
    const p = part.trim()
    if (!p) continue
    const direct = cleanUsername(p)
    if (direct) return direct
    const words = p.split(/\s+/)
    for (const w of words) {
      const u = cleanUsername(w)
      if (u) return u
    }
  }
  return cleanUsername(trimmed)
}

/**
 * Parse pasted seller list (one per line, comma-separated, or eBay profile URLs).
 * @returns {{ rows: Array, rejected: number }}
 */
export function parseSellerUsernameList (text) {
  const seen = new Set()
  const rows = []
  const raw = String(text || '')
  scanEbayUrls(raw, seen, rows)

  for (const line of raw.split(/\r?\n/)) {
    const u = usernameFromLine(line)
    if (u) addRow(seen, rows, u)
  }

  // Also split whole paste by commas/semicolons when user pasted one line
  if (!rows.length && /[,;]/.test(raw)) {
    for (const chunk of raw.split(/[,;]+/)) {
      const u = usernameFromLine(chunk)
      if (u) addRow(seen, rows, u)
    }
  }

  return rows
}
