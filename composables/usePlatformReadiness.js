...
export function usePlatformReadiness () {
  ...
  const checks = computed(() => [
    ...
    { id: "per-item-checkout", label: "Per-item Stripe Checkout", ok: checkoutEnabled.value && hasSupabase.value, detail: checkoutEnabled.value ? "Buy now creates a session for the listing price (Edge Functions on Supabase)." : "Checkout disabled in config.", tier: "transactions" },
  ...
  const readinessSummary = computed(() => {
    if (transactionsOk.value && paymentsOk.value) {
      return "Buyers pay the exact listing price via Stripe Checkout. Orders are tracked; escrow releases when the buyer confirms receipt."
    }
